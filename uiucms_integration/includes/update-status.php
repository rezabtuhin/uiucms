...
function updateStatusMem($con2, $updateId)
{

    $getStatus = getStatusMem($con2, $updateId);
    if (!empty($getStatus)) {

        if ($getStatus['status'] == 0) {
            $sql = "UPDATE members SET status=1 WHERE club_id=$updateId";

            if ($con2->query($sql) === TRUE) {
                echo 1;
            }
        } elseif ($getStatus['status'] == 1) {
            $sql = "UPDATE members SET status=0 WHERE club_id=$updateId";
...
function getStatusMem($con2, $id)
{

    $query = "SELECT status FROM members WHERE club_id=$id";
    $result = $con2->query($query);
